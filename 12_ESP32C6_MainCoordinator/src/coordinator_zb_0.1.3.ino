...
#define HA_THERMOSTAT_ENDPOINT      1                                    /* esp coordinator device endpoint */
...
static void esp_zb_data_req_handler() {
    /* Send "read attributes" command to the bound sensor */
    esp_zb_zcl_read_attr_cmd_t read_req;
    read_req.address_mode = ESP_ZB_APS_ADDR_MODE_DST_ADDR_ENDP_NOT_PRESENT;
    read_req.zcl_basic_cmd.src_endpoint = HA_THERMOSTAT_ENDPOINT;
...
static void esp_app_zb_attribute_handler(uint16_t cluster_id, const esp_zb_zcl_attribute_t *attribute) {
  ...
  if (cluster_id == ESP_ZB_ZCL_CLUSTER_ID_TEMP_MEASUREMENT) {
    ...
    if (attribute->id == ESP_ZB_ZCL_ATTR_TEMP_MEASUREMENT_VALUE_ID && attribute->data.type == ESP_ZB_ZCL_ATTR_TYPE_S16) {
      ...
      value = attribute->data.value ? *(int16_t *)attribute->data.value : -999;
    ...
    if (attribute->id == ESP_ZB_ZCL_ATTR_TEMP_MEASUREMENT_MIN_VALUE_ID && attribute->data.type == ESP_ZB_ZCL_ATTR_TYPE_S16) {
      min_value = attribute->data.value ? *(int16_t *)attribute->data.value : -999;
      data_value_valid = true;
      // log_i("Min Measured Value is %.2f degrees Celsius", zb_s16_to_temperature(min_value));
      // log_i(" ||||| Value: %d ", min_value);
    }

    if (min_value != -999 && data_value_valid) {
      log_i("Data Value: %d", min_value);
      log_i("Data Key: %d", value);

      digitalWrite(LED_BUILTIN, !digitalRead(LED_BUILTIN));

      if (value == 0) {
                      log_i("Chip fridge live bit: %d", min_value);
      }
      else if (value == 1) {
                      log_i("Chip fridge temperature: %d", min_value);
      } else if (value == 2) {
                      log_i("1-wire temperature: %d", min_value);               // freezer temperature
      } else if (value == 3) {
                      log_i("Door sensor: %d", min_value);                      
      } else if (value == 4) {
                      log_i("sht31 temperature: %d", min_value);                // fridge temperature
      } else if (value == 5) {
                      log_i("sht31 humidity: %d", min_value);                   // fridge humidity
      } else if (value == 6) {
                      log_i("Chip fan live bit: %d", min_value);
      } else if (value == 7) {
                      log_i("Chip fan temperature: %d", min_value);
      } else if (value == 8) {
                      log_i("Vibration axis X: %d", min_value);
      } else if (value == 9) {
                      log_i("Vibration axis Y: %d", min_value);
      } else if (value == 10) {
                      log_i("Vibration axis Z: %d", min_value);
      }

      // send data over UART to RPi
      Serial1.print("K:");Serial1.print(value);Serial1.print(":");
      Serial1.print("V:");
      Serial1.print(min_value);Serial1.print(":");
...
void loop() {
  ...
  while(true) {
              ...
              esp_zb_data_req_handler();
              delay(1000);
              log_i(" main while loop ");

              // test how to send data over UART
              //Serial1.print("K:%d:V:%d", 99, 125);
              //Serial1.print((String)"K:" + 99 + ":V:" + 125 + ":");
              
              // get data from serial port
              // if (Serial1.available() > 0) {
              //   String receive_serial =  Serial1.readString();
              //   log_i("Received serial data %s", receive_serial);
