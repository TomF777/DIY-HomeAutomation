...
class ZigbeeGenericCoordinator : public ZigbeeEP {
public:
  ZigbeeGenericCoordinator(uint8_t endpoint);
  ~ZigbeeGenericCoordinator() {}

  // sensor methods
  void onTempReceive(void (*callback)(float)) {
    _on_temp_receive = callback;
  }
  void onSensorValueReceiveWithSource(void (*callback)(float, uint8_t, esp_zb_zcl_addr_t)) {
    _on_temp_receive_with_source = callback;
  }
  // For backward compatibility: keep onConfigReceive as an alias to onSensorConfigReceive (deprecated).
  [[deprecated("Use onSensorConfigReceive instead.")]]
  void onConfigReceive(void (*callback)(float, float, float)) {
    onSensorConfigReceive(callback);
  }
  void onSensorConfigReceive(void (*callback)(float, float, float)) {
  ...
  void onIndexValueReceiveWithSource(void (*callback)(float, uint8_t, esp_zb_zcl_addr_t)) {
    _on_humidity_receive_with_source = callback;
  }
  void onIndexConfigReceive(void (*callback)(float, float, float)) {
...
private:
  ...
  static ZigbeeGenericCoordinator *_instance;
