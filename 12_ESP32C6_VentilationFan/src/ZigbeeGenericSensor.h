...
class ZigbeeGenericSensor : public ZigbeeEP {
public:
  ZigbeeGenericSensor(uint8_t endpoint);
  ~ZigbeeGenericSensor() {}

  // Set the temperature value in 0,01°C
  bool setTemperature(float value);
  
   // Set the generic sensor value in 0,01°C
  bool setSensorValue(uint16_t key_index,  float temperature);
  ...
  // Set the reporting interval for sensor measurement in seconds and delta (temp change in 0,01)
  bool setSensorReporting(uint16_t min_interval, uint16_t max_interval, float delta);
  ...
  void addIndexToSensor(float min, float max, float tolerance);
  ...
  // Set the reporting interval for index in seconds and delta (index value change in 0,01)
  bool setIndexReporting(uint16_t min_interval, uint16_t max_interval, float delta);
