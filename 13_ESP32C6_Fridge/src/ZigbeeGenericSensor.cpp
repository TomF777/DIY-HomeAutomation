...
ZigbeeGenericSensor::ZigbeeGenericSensor(uint8_t endpoint) : ZigbeeEP(endpoint) {
...
static int32_t zb_float_to_s32(float temp) {
  return (int32_t)(temp * 100);
}

bool ZigbeeGenericSensor::setMinMaxValue(float min, float max) {
...
bool ZigbeeGenericSensor::setTolerance(float tolerance) {
...
bool ZigbeeGenericSensor::setReporting(uint16_t min_interval, uint16_t max_interval, float delta) {
...
bool ZigbeeGenericSensor::setSensorReporting(uint16_t min_interval, uint16_t max_interval, float delta) {
  esp_zb_zcl_reporting_info_t reporting_info;
  memset(&reporting_info, 0, sizeof(esp_zb_zcl_reporting_info_t));
  reporting_info.direction = ESP_ZB_ZCL_CMD_DIRECTION_TO_SRV;
  reporting_info.ep = _endpoint;
...
bool ZigbeeGenericSensor::setTemperature(float temperature) {
...
bool ZigbeeGenericSensor::setSensorValue(uint16_t key_index,  float temperature) {
  esp_zb_zcl_status_t ret = ESP_ZB_ZCL_STATUS_SUCCESS;

  int16_t zb_temperature = zb_float_to_s16(temperature);
  uint16_t index = (uint16_t)(key_index * 100);

  //int32_t value_to_send = (uint32_t)key_index << 16;
  //value_to_send = value_to_send | (int32_t)val_s16;
  ...
  ret = esp_zb_zcl_set_attribute_val(
   ...
   ret = esp_zb_zcl_set_attribute_val(
    _endpoint, ESP_ZB_ZCL_CLUSTER_ID_REL_HUMIDITY_MEASUREMENT, ESP_ZB_ZCL_CLUSTER_SERVER_ROLE, ESP_ZB_ZCL_ATTR_REL_HUMIDITY_MEASUREMENT_VALUE_ID, &index,
    false
  );
...
bool ZigbeeGenericSensor::reportTemperature() {
...
void ZigbeeGenericSensor::addHumiditySensor(float min, float max, float tolerance) {
...
void ZigbeeGenericSensor::addIndexToSensor(float min, float max, float tolerance) {
  uint16_t zb_min = (uint16_t)(min * 100);
  uint16_t zb_max = (uint16_t)(max * 100);
  uint16_t zb_tolerance = (uint16_t)(tolerance * 100);
  uint16_t default_hum = ESP_ZB_ZCL_REL_HUMIDITY_MEASUREMENT_MEASURED_VALUE_DEFAULT;
...
bool ZigbeeGenericSensor::setHumidity(float humidity) {
  esp_zb_zcl_status_t ret = ESP_ZB_ZCL_STATUS_SUCCESS;
...
bool ZigbeeGenericSensor::reportHumidity() {
  /* Send report attributes command */
  esp_zb_zcl_report_attr_cmd_t report_attr_cmd;
  report_attr_cmd.address_mode = ESP_ZB_APS_ADDR_MODE_DST_ADDR_ENDP_NOT_PRESENT;
...
bool ZigbeeGenericSensor::setHumidityReporting(uint16_t min_interval, uint16_t max_interval, float delta) {
  esp_zb_zcl_reporting_info_t reporting_info;
  memset(&reporting_info, 0, sizeof(esp_zb_zcl_reporting_info_t));
  reporting_info.direction = ESP_ZB_ZCL_CMD_DIRECTION_TO_SRV;
  reporting_info.ep = _endpoint;
...
bool ZigbeeGenericSensor::setIndexReporting(uint16_t min_interval, uint16_t max_interval, float delta) {
  esp_zb_zcl_reporting_info_t reporting_info;
  memset(&reporting_info, 0, sizeof(esp_zb_zcl_reporting_info_t));
  reporting_info.direction = ESP_ZB_ZCL_CMD_DIRECTION_TO_SRV;
  reporting_info.ep = _endpoint;
...
bool ZigbeeGenericSensor::report() {
